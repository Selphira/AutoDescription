DEFINE_PATCH_MACRO ~update_description~ BEGIN
	...
	ELSE BEGIN
		INNER_ACTION BEGIN
			APPEND_OUTER ~AutoDescription/log/warnings.log~ ~%SOURCE_FILE% : Fichier invalide.~
		END
	END
END
...
DEFINE_PATCH_MACRO ~log~ BEGIN
	...
	INNER_ACTION BEGIN
		SILENT
		APPEND_OUTER ~AutoDescription/log/diff.html~ ~<tr><th class="title">%filename%<br/>%itemName%</th><td class="description before"><pre>%oldDescription%</pre></td><td class="description after"><pre>%description%</pre></td></tr>~ KEEP_CRLF
		VERBOSE
	END
END
